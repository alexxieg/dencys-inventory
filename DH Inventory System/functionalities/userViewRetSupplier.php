...
<html lang="en">
                    ...
                    buttons: [
                        {
							...
							message: 'Supplier Return Details', 
	...
	<body>
		...
		<?php
			$retID= $_GET['retId'];
			$query = $conn->prepare("SELECT product.prodID, returns.receiptNo, returns.returnDate, returns.returnID, product.prodName, CONCAT(returns.returnQty, ' ', product.unitType) AS returnQty, returns.returnRemark, returns.userID  
					FROM returns INNER JOIN product ON returns.prodID = product.prodID
					WHERE returns.receiptNo = '$retID'");
			$query->execute();
			$result = $query->fetchAll();
			
			$query2 = $conn->prepare("SELECT product.prodID, returns.receiptNo, returns.returnDate, returns.returnID, product.prodName, CONCAT(returns.returnQty, ' ', product.unitType) AS returnQty, returns.returnRemark, returns.userID 
					FROM returns INNER JOIN product ON returns.prodID = product.prodID 
					WHERE returns.receiptNo = '$retID'");
			$query2->execute();
			...
			<div class="row navbar-collapse">
				<div class="col-sm-3 col-md-2 sidebar">
					...
					<ul class="nav nav-sidebar">
						...
						<li class="active"><a href="#" data-toggle="collapse" data-target="#returns"><i class="glyphicon glyphicon-retweet"></i> Returns <i class="glyphicon glyphicon-menu-down" id="dropDownArrow"></i></a>
