...
<html lang="en">
	...
	<body>
		...
		<?php
			$retID= $_GET['retId'];
			$query = $conn->prepare("SELECT product.prodID, returns.receiptNo, returns.returnDate, returns.returnID, product.prodName, returns.returnQty, returns.returnRemark, returns.userID  
					FROM returns INNER JOIN product ON returns.prodID = product.prodID
					WHERE returns.receiptNo = '$retID'");
			$query->execute();
			$result = $query->fetchAll();
			
			$query2 = $conn->prepare("SELECT product.prodID, returns.receiptNo, returns.returnDate, returns.returnID, product.prodName, returns.returnQty, returns.returnRemark, returns.userID, returns.receiptNo 
					FROM returns INNER JOIN product ON returns.prodID = product.prodID 
					WHERE returns.receiptNo = '$retID'");
			$query2->execute();
				...
				<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">				
					<div id="contents">
						<div id="tableHeader">
							...
							<!-- Table Display for Supplier Returns -->
